# Hi there, I'm Oksana Fadyeyeva 👋 

### Agile Strategy & Transformation | Project Manager | Scrum Master| Proxy Product Owner | Agile Leader



[![View Portfolio](https://img.shields.io/badge/View_Full_Portfolio-Click_Here-blueviolet?style=for-the-badge&logo=rocket)](https://pmoksana.github.io/Portfolio-PM-PSM-PAL/)

---

> Professional IT Leader with 5+ years of hybrid experience as a Project manager, Product Manager, Proxy Product owner. Data Science Enthusiast.
I bridge the gap between business and teams.

---



> **My Philosophy:** I lead teams through a purposeful evolution: from **Coaching** and **Supporting** to **Delegating**. My goal is to transform squads from "command-and-control" environments into high-performing, autonomous units rooted in psychological safety and ownership.

---

##   Certifications
| | | | |
| :---: | :---: | :---: | :---: |
| Agile Leadership | Scrum Master | OCI Data Science | AWS Cloud Practitioner | Cisco Security |
| :---: | :---: | :---: | :---: | :---: |
| [![PAL I](https://img.shields.io/badge/Scrum.org-PAL_I-006699?style=flat&logo=scrum.org)](https://www.credly.com/badges/e95fa802-58e6-4145-a830-e1ae42e28293/) | [![PSM I](https://img.shields.io/badge/Scrum.org-PSM_I-006699?style=flat&logo=scrum.org)](https://www.credly.com/badges/0adcae72-5cd0-46c3-ad04-a117162b5eb9/) | [![Oracle](https://img.shields.io/badge/Oracle-Data_Science-F80000?style=flat&logo=oracle)](https://catalog-education.oracle.com/ords/certview/sharebadge?id=1616454E45C1D963F9B7B72E5630E047AA586F75AB555BDC4F8B1FDF93EFC061) | [![AWS](https://img.shields.io/badge/AWS-Certified-FF9900?style=flat&logo=amazon-aws)](https://www.credly.com/badges/77c7532f-a6be-43d4-bf8b-55e50dd50837/) | [![Cisco](https://img.shields.io/badge/Cisco-Security-118811?style=flat&logo=cisco)](https://www.credly.com/badges/af760351-f2f7-4417-9001-d94825d75f6b/) |
| **Agile Leadership**  | **Scrum Master**  | **OCI Data Science**  | **AWS Cloud Practitioner**  | **Cisco Security**


---

## 🚀 Transformation & Coaching Flexibility
I specialize in the lifecycle of team maturity, moving away from rigid oversight toward empowerment:

* **Fostering Ownership:** Scaling agile maturity by facilitating a culture of autonomy and empowerment within engineering teams.
* **Psychological Safety:** Utilizing empathetic leadership and conflict resolution to build trust.
* **Data-Driven Growth:** Leveraging **DORA metrics** and Agile KPIs (Velocity, Cycle Time, Throughput) to guide team growth without micromanagement.
* **Continuous Improvement:** Driving initiatives that resulted in a **25% boost** in team adoption and dynamics.

---

## 📈 Impact by the Numbers
* **Agile Maturity:** Increased sprint predictability by **20%** via custom real-time dashboards.
* **Efficiency:** Reduced non-value-add time by **18%** by removing complex blockers and impediments.
* **Lean Operations:** Improved team throughput by **20%** through Kanban WIP limit implementation.
* **Quality:** Championed "Invest in Quality" sprints, reducing critical post-deployment bugs by **20%**.

---

## 🛠️ Tech Stack & Learning Path
I am an active self-learner in **Data Science** and **Cloud Infrastructure**, ensuring I can "speak the language" of the squads I lead..

* **Management:** Jira, Azure DevOps, ClickUp, Asana, Confluence.
* **Data & AI:** SQL, Tableau, Power BI, AWS Redshift, Jupyter Notebooks.
* **Ecosystems:** Salesforce  (100+ epics/stories managed).


---

##  Key Performance Metrics
* **Predictability:** Increased sprint predictability by **20%** using custom Power BI dashboards.
* **Efficiency:** Reduced non-value-add time by **18%** through blocker removal.
* **Optimization:** Decreased reporting time by **40%** via automated Jira/Confluence templates.
* **Quality:** Achieved a **20% reduction** in post-deployment critical bugs.

---

## 🛠 Tech Stack & Tools

### 🏗 Project Management & CRM

![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat&logo=jira&logoColor=white) ![Salesforce](https://img.shields.io/badge/Salesforce-00A1E0?style=flat&logo=salesforce&logoColor=white) ![Azure DevOps](https://img.shields.io/badge/Azure_DevOps-0078D7?style=flat&logo=azure-devops&logoColor=white) ![Asana](https://img.shields.io/badge/Asana-273347?style=flat&logo=asana&logoColor=white) ![ClickUp](https://img.shields.io/badge/ClickUp-7B68EE?style=flat&logo=clickup&logoColor=white) 

### 📈 Data & Analytics

![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white) ![PowerBI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=microsoft-power-bi&logoColor=black) ![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white) ![Redshift](https://img.shields.io/badge/AWS_Redshift-8C4FFF?style=flat&logo=amazon-redshift&logoColor=white) 

### 🎨 Collaboration & Design

![Miro](https://img.shields.io/badge/Miro-050038?style=flat&logo=miro&logoColor=white) ![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white) ![Confluence](https://img.shields.io/badge/Confluence-172B4D?style=flat&logo=confluence&logoColor=white)  

---

## 🎓 Education
* **Master's in Computer Science** | Simon Kuznets Kharkiv National University of Economics (2020) .

---

## 📬 Connect with Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/oksana-fadyeyeva)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pmoksana.f@gmail.com)

